module testbench();
parameter STAGES    = 2;  // Parameter for complexity research purposes
parameter SIG_WIDTH = 128*1;  // parameter value generated by script
...
initial begin
  init_timer();
  cntr = 0;
  repeat(STAGES+100000) @(posedge clk)#1cntr++;
  print_time(cntr);
  $display(cntr);
  $finish();
end
...
assert property (
    ...
    sig_out_dut == $past(sig_inverted, STAGES)
);
//===============================
